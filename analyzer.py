...
def get_week_stats(telegram_id: int) -> str:
    ...
    return (
        f"📅 *Посмотреть статистику за неделю*\n\n"
        f"📊 Всего записей: *{cnt}*\n\n"
        f"😊 *Настроение:*\n"
...
def get_month_stats(telegram_id: int) -> str:
    ...
    if not stats or stats.get("cnt", 0) == 0:
        return (
            "🗓 *Посмотерть статистику за месяц*\n\n"
            "Записей пока нет \n + Записать день"
        )
    ...
    return (
        f"🗓 *Посмотреть статистику за месяц*\n\n"
        f"📊 Всего записей: *{cnt}*\n\n"
        f"😊 *Настроение:*\n"
        f"   Среднее: {_mood_label(stats['avg_mood'])}\n"
        f"   Мин: {stats['min_mood']}  |  Макс: {stats['max_mood']}\n\n"
...
def get_insights(telegram_id: int) -> str:
    ...
    if not entries:
        return (
            " *Мои интересные факты*\n\n"
            "Мало данных для анализа\n"
            "Добавь еще записей"
        )
    lines = [" *Мои интересные факты за месяц*\n"]
    ...
    if len(lines) == 1:
        lines.append("Добавь больше записей для анализа")
    return "\n".join(lines)
...
def get_chart(telegram_id: int) -> io.BytesIO | None:
    ...
    fig.suptitle("📊 Твой трекер за последний месяц", fontsize=14,
