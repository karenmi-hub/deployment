...
def cmd_start(message):
    ...
    text = (
        "👋 *Привет! Я твой личный трекер самочувствия.*\n\n"
        "Я помогаю отслеживать три ключевых показателя каждый день:\n"
        "😊 *Настроение* — от 1 до 5\n"
        "📚 *Часы работы/учёбы*\n"
        "😴 *Часы сна*\n\n"
        "На основе твоих данных я выявляю закономерности:\n"
        "_Когда ты наиболее продуктивен? Как сон влияет на настроение?_\n\n"
        "Можно добавлять *неограниченное количество* записей в день!\n\n"
...
def cmd_help(message):
    text = (
        ...
        "📊 *Статистика* — сводка и аналитика\n"
        "📜 *История* — все твои записи\n"
        "⚙️ *Настройки* — время напоминания\n"
        "❓ *Помощь* — эта справка\n\n"
        "*/clear* — очистить все данные\n\n"
        "💡 *Совет:* добавляй записи несколько раз в день — "
        "утром, днём и вечером — для более точной картины!"
    )
    bot.send_message(message.chat.id, text, parse_mode="Markdown", reply_markup=main_keyboard())
...
def cmd_settings(message):
    ...
    reminder = settings.get("reminder_time", "20:00") if settings else "20:00"
    text = (
        f"⚙️ *Настройки*\n\n"
...
def cmd_clear(message):
    ...
    bot.send_message(
        message.chat.id,
        "⚠️ *Вы уверены?*\n\nВсе твои записи будут *безвозвратно удалены*.",
        parse_mode="Markdown",
    ...
    if state == STATE_WORK_CUSTOM:
        ...
        if re.match(r"^\d{2}:\d{2}$", text):
            ...
            if 0 <= h <= 23 and 0 <= m <= 59:
                ...
                return
        bot.send_message(message.chat.id, "❌ Неверный формат. Введи время как `ЧЧ:ММ`, например `20:00`", parse_mode="Markdown")
...
def handle_callback(call):
    ...
    if data.startswith("mood_"):
        ...
        bot.send_message(call.message.chat.id, "Начнём заново!", reply_markup=main_keyboard())
...
def save_entry(chat_id, uid, comment):
    ...
    text = (
        ...
        f"😴 Сон: *{sleep} ч*"
        f"{comment_text}\n\n"
        f"_Можешь добавить ещё одну запись в любое время!_"
    )
...
def send_history_page(chat_id, uid, page, message_id=None):
    ...
    if not entries:
        msg = "📜 *История пуста.*\n\nДобавь первую запись через ➕ Записать день!"
        if message_id:
...
def send_reminders():
    ...
    for user in users:
        try:
            bot.send_message(
                user["telegram_id"],
                "🔔 *Напоминание!*\n\nНе забудь записать свой день сегодня 😊",
                parse_mode="Markdown",
                reply_markup=main_keyboard()
            )
        except Exception:
